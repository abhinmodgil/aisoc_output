...
GITHUB_BRANCH = "main"
# Base path for alert-specific subdirectories (e.g., outputs/alerts/ALERT_ID/)
ALERTS_DATA_BASE_PATH = "outputs/alerts"
# Path for the Wazuh alerts JSON file (JSON Lines format) at the REPOSITORY ROOT
...
def list_github_directories(username, repo, path, branch, token):
    """Lists directories from a GitHub repository path."""
    ...
    try:
        ...
        st.error(f"HTTP error listing dirs for '{path}': {http_err}")
...
def get_github_file_content(username, repo, file_path, branch, token):
    """Fetches and decodes file content from a GitHub file_path via API."""
    ...
    try:
        ...
        if "content" in content_data:
            ...
            st.error(f"No 'content' in API response for '{file_path}'. JSON: {content_data}");
            return None
    except requests.exceptions.HTTPError as http_err:
        # For 404, we don't want a big red error, just return None.
        if response_api.status_code == 404:
            # st.info(f"File not found (404): '{file_path}'") # Optional: log for debugging
            return None
        st.error(f"HTTP error getting file '{file_path}': {http_err}")
    except Exception as e:
...
def load_wazuh_alerts_data_from_github(username, repo, file_path, branch, token):
    """
    Fetches Wazuh alerts JSON (JSON Lines format) from GitHub.
    Each line in the file is expected to be a separate JSON object.
    Returns a tuple: (parsed_alerts_dict, error_message, raw_content_if_error)
    """
    json_lines_content_str = get_github_file_content(username, repo, file_path, branch, token)
    ...
    if json_lines_content_str:
        ...
        elif json_lines_content_str.strip():
        ...
        return None, f"Could not retrieve content for Wazuh alerts file: '{file_path}'", None
...
def parse_phase1_ioc_file_content(raw_text):
    ...
    # Attempt to find the "Validated IOCs:" block first, as it's cleaner
    ...
    if validated_iocs_block_match:
        ...
        # Fallback to the raw LLM output block if "Validated IOCs:" is not found
        ...
        else:  # If neither is found, use the whole raw text
    ...
    for line in lines:
        ...
        if not line or line.startswith("---") or line.startswith(
            "Extracted IOCs and Entities:"): continue  # Skip headers or separators
...
def parse_phase1_5_contextual_inquiry_file(raw_text):
    ...
    # Remove the top-level file header first
    ...
    if assessment_content_start != -1 and questions_content_start != -1:
        if assessment_content_start < questions_header_match.start():  # Assessment before questions
        ...
        else:  # Questions before assessment (unlikely but handle)
    ...
    elif assessment_content_start != -1:  # Only assessment header found
        initial_assessment = text_to_parse[assessment_content_start:].strip()
        questions_block = ""
    elif questions_content_start != -1:  # Only questions header found
        initial_assessment = text_to_parse[:questions_header_match.start()].strip()  # Text before it
        questions_block = text_to_parse[questions_content_start:].strip()
    else:  # No specific headers found, assume entire content is assessment-like
        initial_assessment = text_to_parse.strip()
        questions_block = ""
        # st.info("Phase 1.5: Could not find standard assessment/questions headers. Displaying content as assessment.")
    ...
    if questions_block:
        ...
        if not questions_list_strings and questions_block.strip():  # If regex fails but block has content
            # Fallback: split by line and take non-empty lines as questions
...
def parse_phase2_plan_file_content(raw_text):
    ...
    # --- Extract overall headers (Alert ID, Agent ID) ---
    ...
    initial_assessment = initial_assessment_match.group(
        1).strip() if initial_assessment_match else "Assessment not parsed."

    # --- Extract Validated IOCs ---
    validated_iocs_match = re.search(
        r"--- Validated IOCs Used for Plan Generation ---\s*(.*?)\s*--- Detailed Investigative Questions and Plans ---",
        raw_text, re.DOTALL
    )
    validated_iocs_text = validated_iocs_match.group(
        1).strip() if validated_iocs_match else "Validated IOCs not parsed."

    # --- Extract Individual Questions and Plans ---
    questions_and_plans = []
    # The block containing all questions and plans
    ...
    if plans_block_match:
        ...
        # Regex to find each Question + Plan block
        # It looks for "Question X:" followed by "Detailed Plan:" and then the plan content
        # until the next "Question X:" or the end of the string, handling the separator.
        ...
        for match in individual_plan_pattern.finditer(plans_content):
            ...
            # Remove the trailing "--------------------------------------------------" if present at the very end of this plan_text
            if plan_text.endswith("\n" + "-" * 50):
                plan_text = plan_text[:-len("\n" + "-" * 50)].strip()
            elif plan_text.endswith("-" * 50):  # If no newline before separator
    ...
    if not questions_and_plans and plans_block_match and plans_block_match.group(1).strip():
        # If the main block was found, but individual Q&P regex failed, check for error/note
...
all_wazuh_alerts_map, wazuh_load_error_msg, wazuh_error_raw_content = load_wazuh_alerts_data_from_github(
    GITHUB_USERNAME, GITHUB_REPO_NAME, WAZUH_ALERTS_FILE_PATH, GITHUB_BRANCH, GH_TOKEN_FOR_REQUESTS
)
...
if all_wazuh_alerts_map is not None and not wazuh_load_error_msg:
    ...
    st.warning(f"No alerts with 'id' fields parsed from '{WAZUH_ALERTS_FILE_PATH}'.")

alert_id_dirs = list_github_directories(
    GITHUB_USERNAME, GITHUB_REPO_NAME, ALERTS_DATA_BASE_PATH, GITHUB_BRANCH, GH_TOKEN_FOR_REQUESTS
)
...
if not selected_alert_id:
    ...
    with tabs[0]:
        ...
        if original_alert_data_to_display:
            ...
            st.warning(f"Original Wazuh alert for ID '{selected_alert_id}' not found.")
        else:
    ...
    if len(tabs) > 1:
        with tabs[1]:
            ...
            p1_ioc_file_content = get_github_file_content(GITHUB_USERNAME, GITHUB_REPO_NAME, p1_ioc_file_path,
                                                          GITHUB_BRANCH, GH_TOKEN_FOR_REQUESTS)
            ...
            elif alert_id_dirs:  # Only show error if dirs were loaded but file is missing
                st.error(f"Could not load P1 IOC file: '{p1_ioc_file_path}'. File might not exist for this alert.")
            # No "else" needed if alert_id_dirs is empty, as the main select box handles that.
    ...
    if len(tabs) > 2:
        with tabs[2]:
            ...
            p1_5_context_file_content = get_github_file_content(
                GITHUB_USERNAME, GITHUB_REPO_NAME, p1_5_context_file_path, GITHUB_BRANCH, GH_TOKEN_FOR_REQUESTS
            )
            if p1_5_context_file_content:
                ...
                if initial_assessment:
                    ...
                    st.markdown(initial_assessment)  # Markdown will render **bold**
                else:
                    st.markdown("**Initial Alert Assessment and Dimensional Focus:**")
                ...
                st.info(
                    f"Phase 1.5 Contextual Inquiry file not found: '{p1_5_context_file_path}'. File might not exist or could not be loaded for this alert.")

    # --- Tab 3: Phase 2 - Investigative Plans ---
    if len(tabs) > 3:
        with tabs[3]:
            st.subheader("📝 Generated Investigative Plans")
            # Corrected file name based on your Kaggle cell
            ...
            p2_plans_file_content = get_github_file_content(
                GITHUB_USERNAME, GITHUB_REPO_NAME, p2_plans_file_path, GITHUB_BRANCH, GH_TOKEN_FOR_REQUESTS
            )
            if p2_plans_file_content:
                ...
                if phase2_assessment and phase2_assessment != "Assessment not parsed.":
                    ...
                    st.markdown(phase2_assessment)  # This is markdown-friendly
                else:
                ...
                if phase2_iocs and phase2_iocs != "Validated IOCs not parsed.":
                    ...
                    # Try to parse as JSON if it looks like it, otherwise display as text block
                    try:
                        iocs_json = json.loads(phase2_iocs)
                        st.json(iocs_json)
                    except json.JSONDecodeError:
                        st.text_area("IOCs Text:", value=phase2_iocs, height=150, disabled=True,
                                     key=f"p2_iocs_{selected_alert_id}")
                else:
                ...
                if phase2_q_and_plans:
                    ...
                    for i, plan_data in enumerate(phase2_q_and_plans):
                        with st.container():  # Use container for better visual separation
                            ...
                            # The plan_text from Phase 2 output often contains markdown for structure
                            st.markdown(plan_data.get('plan_text', "No plan details parsed."))
                            if i < len(phase2_q_and_plans) - 1:
                                st.markdown("---")  # Horizontal line between plans
                else:
                    st.info(
                        "No individual plans were parsed from the Phase 2 file. The 'Detailed Investigative Questions and Plans' section might be empty, malformed, or only contain a note.")
                ...
                st.info(
                    f"Phase 2 plans file not found: '{p2_plans_file_path}'. File might not exist or could not be loaded for this alert.")
